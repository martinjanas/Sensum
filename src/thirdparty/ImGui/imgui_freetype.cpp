...
namespace
{
    ...
    struct FreeTypeFont
    {
        ...
        void                    BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, int* glyph_padding, unsigned char* multiply_table = nullptr);
    ...
    void FreeTypeFont::BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, int* glyph_padding, unsigned char* multiply_table)
    {
        ...
        for (int i = 0; i < (w + glyph_padding[1] + glyph_padding[3]) * (h + glyph_padding[0] + glyph_padding[2]); i++) // fill padding with transparent white
            dst[i] = 0xFFFFFF;

        dst += glyph_padding[0] * dst_pitch;
                ...
                if (multiply_table == nullptr)
                {
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                        for (uint32_t x = 0; x < w; x++)
                            dst[x + glyph_padding[3]] = IM_COL32(255, 255, 255, src[x]);
                }
                else
                {
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                        for (uint32_t x = 0; x < w; x++)
                            dst[x + glyph_padding[3]] = IM_COL32(255, 255, 255, multiply_table[src[x]]);
                }
                break;
            }
        case FT_PIXEL_MODE_MONO: // Monochrome image, 1 bit per pixel. The bits in each byte are ordered from MSB to LSB.
            {
                ...
                for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                {
                    ...
                    for (uint32_t x = 0; x < w; x++, bits <<= 1)
                    {
                        if ((x & 7) == 0)
                            bits = *bits_ptr++;
                        dst[x + glyph_padding[3]] = IM_COL32(255, 255, 255, (bits & 0x80) ? color1 : color0);
                    }
                }
                break;
            }
                ...
                if (multiply_table == nullptr)
                {
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                        for (uint32_t x = 0; x < w; x++)
                        {
                            ...
                            dst[x + glyph_padding[3]] = IM_COL32(DE_MULTIPLY(r, a), DE_MULTIPLY(g, a), DE_MULTIPLY(b, a), a);
                        }
                }
                else
                {
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                    {
                        ...
                        {
                            uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4], a = src[x * 4 + 3];
                            dst[x + glyph_padding[3]] = IM_COL32(multiply_table[DE_MULTIPLY(r, a)], multiply_table[DE_MULTIPLY(g, a)], multiply_table[DE_MULTIPLY(b, a)], multiply_table[a]);
                        }
                    }
                }
                #undef DE_MULTIPLY
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
        {
            ...
            src_glyph.Info.Width += cfg.GlyphPadding[1] + cfg.GlyphPadding[3];
            src_glyph.Info.Height += cfg.GlyphPadding[0] + cfg.GlyphPadding[2];
            src_glyph.Info.OffsetX += cfg.GlyphPadding[3];
            src_glyph.Info.OffsetY += cfg.GlyphPadding[0];
            ...
            src_tmp.Font.BlitGlyph(ft_bitmap, src_glyph.BitmapData, src_glyph.Info.Width, cfg.GlyphPadding, multiply_enabled ? multiply_table : nullptr);
