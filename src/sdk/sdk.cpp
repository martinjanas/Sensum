...
namespace sdk
{
	void scan_and_cache_sigs()
	{
		...
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "48 8B 1D ?? ?? ?? ?? 48 8D 15 ?? ?? ?? ?? 48 8D 4D 40 66 89 44 24 60", "g::engine_trace", 0x3, true));
		...
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "0F 84 D5 00 00 00 48 89 74 24 28", "g::game_rules", 0, false));
		
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "48 89 5C 24 08 48 89 74 24 10 57 48 81 EC 40 01 00 00 8B DA 48 8B F9 E8 ?? ?? ?? ?? 48", "CBaseEntity::GetHitboxSet", 0, false));
		...
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "40 55 56 48 8D 6C 24 B1 48 81 EC 98", "IsInSmoke", 0, false));
		...
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "48 8B C4 48 89 68 08", "hooks::GetMatricesForView", 0, false));
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "40 55 53 56 41 56 41 57 48 8B", "hooks::CalcViewModel", 0, false));
		futures.push_back(std::async(std::launch::async, scan_and_cache_sig, &modules::client, "4C 8B DC 49 89 5B 10 49 89 6B 18 56 57 41 56 48 81 EC F0", "hooks::OnRenderStart", 0, false));	
